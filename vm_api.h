/*
 * vm_api.h
 * Functions exposed by PIGPIO's PIGS VM.
 *
...
extern int gpioSetPullUpDown(unsigned gpio, unsigned pud);
extern int gpioRead(unsigned gpio);
extern int gpioWrite(unsigned gpio, unsigned level);

/* PWM commands */
extern int gpioPWM(unsigned user_gpio, unsigned dutycycle);
extern int gpioSetPWMfrequency(unsigned user_gpio, unsigned frequency);
extern int gpioSetPWMrange(unsigned user_gpio, unsigned range);
extern int gpioGetPWMdutycycle(unsigned user_gpio);
extern int gpioGetPWMfrequency(unsigned user_gpio);
extern int gpioGetPWMrange(unsigned user_gpio);
extern int gpioGetPWMrealRange(unsigned user_gpio);

/* Servo commands */
extern int gpioServo(unsigned user_gpio, unsigned pulsewidth);
extern int gpioGetServoPulsewidth(unsigned user_gpio);

/* Intermediate commands */
extern int gpioTrigger(unsigned user_gpio, unsigned pulseLen, unsigned level);
extern int gpioSetWatchdog(unsigned user_gpio, unsigned timeout);
extern unsigned gpioRead_Bits_0_31(void);
extern unsigned gpioRead_Bits_32_53(void);
extern int gpioWrite_Bits_0_31_Clear(unsigned bits);
extern int gpioWrite_Bits_32_53_Clear(unsigned bits);
extern int gpioWrite_Bits_0_31_Set(unsigned bits);
extern int gpioWrite_Bits_32_53_Set(unsigned bits);

/* Advanced commands */
extern int gpioNotifyOpen(void);
extern int gpioNotifyClose(unsigned handle);
extern int gpioNotifyBegin(unsigned handle, unsigned bits);
extern int gpioNotifyPause(unsigned handle);
extern int gpioHardwareClock(unsigned gpio, unsigned clkfreq);
extern int gpioHardwarePWM(unsigned gpio, unsigned PWMfreq, unsigned PWMduty);
extern int gpioGlitchFilter(unsigned user_gpio, unsigned steady);
extern int gpioNoiseFilter(unsigned user_gpio, unsigned steady, unsigned active);
extern int gpioSetPad(unsigned pad, unsigned padStrength);
extern int gpioGetPad(unsigned pad);

/* Event commands */
extern int eventMonitor(unsigned handle, unsigned bits);
extern int eventTrigger(unsigned event);

/* I2C commands */
extern int i2cOpen(unsigned i2cBus, unsigned i2cAddr, unsigned i2cFlags);
extern int i2cClose(unsigned handle);
extern int i2cWriteQuick(unsigned handle, unsigned bit);
extern int i2cReadByte(unsigned handle);
extern int i2cWriteByte(unsigned handle, unsigned bVal);
extern int i2cReadByteData(unsigned handle, unsigned i2cReg);
extern int i2cWriteByteData(unsigned handle, unsigned i2cReg, unsigned bVal);
extern int i2cReadWordData(unsigned handle, unsigned i2cReg);
extern int i2cWriteWordData(unsigned handle, unsigned i2cReg, unsigned wVal);
extern int i2cProcessCall(unsigned handle, unsigned i2cReg, unsigned wVal);

/* Utility commands */
extern unsigned gpioHardwareRevision(void);
extern unsigned gpioDelay_us(unsigned micros);
extern unsigned gpioDelay_ms(unsigned millis);
extern unsigned gpioVersion(void);
extern unsigned gpioTick(void);

/* Configuration commands */
extern unsigned gpioCfgGetInternals(void);
...
extern int p0, p1, p2, p3, p4, p5, p6, p7, p8, p9;
